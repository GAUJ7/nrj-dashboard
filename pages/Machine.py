...
df2 = pd.read_csv("pages/20241127 Machine_streamlit.csv", sep=";")
...
if site_selection == 'Global':
    ...
    if machine_selection == 'Global':
        df_filtered = df2.groupby([period_choice, 'Machine', site_selection])[energie_choice].sum().reset_index()
...
if period_choice == 'Année':
    ...
    end_week = st.sidebar.selectbox("Sélectionner la semaine de fin", sorted(df2['Semaine_Formate'].unique(), key=lambda x: (int(x.split()[1]), int(x.split()[0][1:]))), index=88)
